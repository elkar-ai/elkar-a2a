...
use crate::{
    ...
    models::{
        ...
        task_subscription::{TaskEventSubscription, TaskEventSubscriptionInput, TaskSubscription},
    },
};
...
pub async fn dequeue_task_event(
    ...
    conn.transaction(|conn| {
        Box::pin(async move {
            ...
            let events = events.load::<(Uuid, Uuid, Value)>(conn).await?;

            let mut successful_ids = Vec::new();
            let mut failed_ids = Vec::new();
            let mut outputs = Vec::new();

            for (id, task_id, event_data) in events {
            ...
            if !successful_ids.is_empty() {
                diesel::update(task_event_subscription::table)
                    ...
                    .set(task_event_subscription::status.eq(TaskEventSubscriptionStatus::Delivered))
                    .execute(conn)
                    .await?;
            }
...
mod tests {
    ...
    async fn setup_test_data(conn: &mut AsyncPgConnection) -> (Uuid, Uuid, Uuid) {
        // Create a task first
